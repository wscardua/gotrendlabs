...
- `GET /health` deve permanecer disponível como healthcheck público read-only e pode ser consumido pelo Admin Ops e pelo app mobile para indicar se a Backend API está online, sem autenticação e sem efeitos colaterais.
- `GET /health` inclui `maintenance`, `checks` e `mobile`; o bloco `mobile` informa versão/build instalados quando enviados por headers, release Android ativa, build mínimo/recomendado, `update_required`, `update_available`, `download_url` e mensagem de atualização obrigatória.
- Compatibilidade mobile é controlada pela API atual única, sem rotas `/api/v1` ou `/api/v2`; `versionCode`/build number é a autoridade e `versionName` é apenas informativo.
