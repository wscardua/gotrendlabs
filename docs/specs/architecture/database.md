...
- Os acessos atuais de integridade possuem indices para sequencia/hash do ledger, FKs de mercado/prova, unicidade de definicao/Seal/compromisso e fila por `(market_id, status)`. Esses indices atendem buscas pontuais e ordenacao indexada, mas nao eliminam o custo `O(E)` de validar todos os eventos nem round-trips por item; checkpoint/cache validado ou verificacao incremental exige desenho futuro sem enfraquecer a prova integral.
