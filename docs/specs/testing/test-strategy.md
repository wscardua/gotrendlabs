...
- A regressao deve provar que nenhuma divergencia aplicavel, inclusive cadeia global invalida ou definicao ausente em estado publicado, permite `valid=true` ou Seal; metadados persistidos do evento tambem devem estar cobertos pela assinatura.
...
- A verificacao publica deve ter plano de carga com volume representativo e `EXPLAIN (ANALYZE, BUFFERS)`: indices existentes sao verificados para lookup/ordenacao, enquanto o custo linear da cadeia global, consultas por evento e consultas por card permanecem riscos separados a medir.
