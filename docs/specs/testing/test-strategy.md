...
- O hardening produtivo do ledger deve testar grants e ownership com roles equivalentes as reais: runtime sem `UPDATE`/`DELETE`/`TRUNCATE`, migrations executadas por role separada, triggers uniformes e nenhuma cascade destrutiva.
- A validacao operacional deve incluir teste sintetico de entrega dos alarmes, dimensoes CloudWatch identicas as metricas publicadas, deteccao de `INSUFFICIENT_DATA`, headers defensivos na FastAPI e fluxo auditado/idempotente de encerramento ou reprocessamento de push terminal.
- O ensaio de capacidade deve acompanhar memoria/swap/CPU/disco da EC2, memoria/conexoes/storage do RDS e latencia da API/daemon, registrando limites objetivos para escalar host, banco ou separar workloads.
