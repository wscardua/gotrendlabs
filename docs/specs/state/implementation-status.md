...
| FEAT-MARKET-001 | draft | parcial | 2026-05-19 | Feed público vem da FastAPI/Postgres, com cards, mini gráficos, destaque principal por `view_count` excluindo `draft`/`canceled`, ticket de onboarding por `view_count` excluindo `draft`/`canceled`, fallback visual de thumbnail, ordenações rápidas client-side, recorte de resolvidos, métrica de previsões totais reais e contadores operacionais de popularidade |
