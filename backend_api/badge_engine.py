...
class BadgeAwardEngine:
    ...
    def _award_by_code(cls, cursor, user_id, code, reason_snapshot=""):
        cursor.execute("SELECT id, name FROM orynth_badge_definitions WHERE code = %s", (code,))
        badge = cursor.fetchone()
        if not badge:
            return False
        cursor.execute(
            """
            INSERT INTO orynth_user_badge_awards (user_id, badge_id, awarded_at, reason_snapshot)
            VALUES (%s, %s, %s, %s)
